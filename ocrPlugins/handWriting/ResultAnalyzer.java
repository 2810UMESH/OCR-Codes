...
public class ResultAnalyzer
{
    ...
    private ArrayList<OCRIdentification> neuralCharIdentList;
    ...
    public ResultAnalyzer(ArrayList<OCRIdentification> mseCharIdentList,
            ArrayList<OCRIdentification> neuralCharIdentList,
            ArrayList<OCRIdentification> aspectCharIdentList)
    {
        this.mseCharIdentList = mseCharIdentList;
        this.neuralCharIdentList = neuralCharIdentList;
    ...
    private ArrayList<CandidateOCRChoice> getCharChoices()
    {
        ...
        for (int i = 0; i < listSize; i++)
        {
            ...
            if (!neuralCharIdentList.isEmpty())
            {
                OCRIdentification currIdent = neuralCharIdentList.get(i);
                cand.addOCRIdent(currIdent);
            }
    ...
    private int getListSize()
    {
        ...
        if (neuralCharIdentList.size() > size)
        {
            size = neuralCharIdentList.size();
        }
        return size;
    }
